...
ifeq ($(OS),win32)
    ...
    TEST_COMMAND := set PYTHONPATH=$(PYTHONPATH) && poetry run pytest -c pyproject.toml --cov-report=html --cov=rgp tests/
else
	PYTHONPATH := `pwd`
    TEST_COMMAND := PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=rgp tests/
endif
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 rgp tests

check-safety:
	poetry check
	poetry run safety check --full-report
	poetry run bandit -ll --recursive rgp tests
